...
#------------------------------------------------------------------------------------------------------------------
# This R script provides code for fitting a non linear fit for the relationship between Beq and analyte concentration
# in order to derive the KD of a reaction. 

# Start by making vectors for the concentration variable and corresponding Beq values.The first value should be 0 
# for both variables 
...
plot(c,Beq, ylim = c(0,300),xlab="[analyte]", ylab="Beq")
      ...
      #choose which range of X-values are most suitable to visualize your curve
...
#Create a data frame of the values of "c" for which you want to get a predictions of the fit.   
new.data <- data.frame(c = seq(min(c),max(c),len = 500)) 
      #Adjust "len =" according to the highest analyte concentration of your data                                
