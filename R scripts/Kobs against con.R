...
#------------------------------------------------------------------------------------------------------------------
# This R script provides code for fitting a linear fit for the relationship between Kobs and analyte concentration
# in order to derive the KD of a reaction. 

# Start by making vectors for the concentration variable and corresponding Kobs values.
...
plot(c,kobs, ylim = c(0,0.03),xlab="[analyte]", ylab="Kobs")
      ...
      #choose which range of X-values are most suitable to visualise your curve

#Create a linear fit for the curve using the "lm" function 
fit <- lm(kobs ~ c)

#visualise the fit in your plot 
abline(fit, col = "red")
